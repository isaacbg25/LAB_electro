...
nx, ny = 132, 132
V_t = np.zeros((ny, nx))
fixed = np.zeros_like(V_t, dtype=bool)  # NoV_ta màscara de punts fixos
...
# Assignem V_talors i marquem com fixos
V_t[int(ny/2+e):int(ny/2+e+l), int(nx/2 - l1-g):int(nx/2 - l1)] = 7.5
fixed[int(ny/2+e):int(ny/2+e+l), int(nx/2 - l1-g):int(nx/2 - l1)] = True

V_t[int(ny/2-e-l):int(ny/2-e), int(nx/2 - l1-g):int(nx/2 - l1)] = 7.5
fixed[int(ny/2-e-l):int(ny/2-e), int(nx/2 - l1-g):int(nx/2 - l1)] = True

V_t[int(ny/2-g//2):int(ny/2+g//2), int(nx/2 + l2-g//2):int(nx/2 + l2+g//2)] = -7.5
...
threshold = np.percentile(E_t, 60)  # només el 15% més fort
...
def representar_segments_llargs(equipotencials, salt=2, llargada=0.5):
    ...
    #plt.streamplot(X_t, Y_t, Ex_t, Ey_t, color='blue', density=1.2, linewidth=0.8, arrowsize=1)rectangle_1 = patches.Circle((3, 0), 0.5, facecolor='black', label='Fil a -7.5 V')
    rectangle_1 = patches.Circle((3, 0), 0.5, facecolor='black', label='Fil a -7.5 V')
    plt.gca().add_patch(rectangle_1)

    rectangle_1 = patches.Rectangle((-1.9, 0.2), 0.4,7.2, facecolor='black', alpha=0.5, label='Placa a 7.5 V')
    plt.gca().add_patch(rectangle_1)

    rectangle_1 = patches.Rectangle((-1.9, -7.4), 0.4,7.2, facecolor='black', alpha=0.5)
    plt.gca().add_patch(rectangle_1)
