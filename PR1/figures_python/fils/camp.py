...
V_t[int(ny/2-l/2):int(ny/2+l/2), int(nx/2 - d/2):int(nx/2 - d/2)+l] = 7.5  # placa esquerra
V_t[int(ny/2-l/2):int(ny/2+l/2), int(nx/2 + d/2):int(nx/2 + d/2)+l] = -7.5    # placa dreta
...
for _ in range(6000):
    ...
    V_t_new[int(ny/2-l/2):int(ny/2+l/2), int(nx/2 - d/2):int(nx/2 - d/2)+l] = 7.5
    V_t_new[int(ny/2-l/2):int(ny/2+l/2), int(nx/2 + d/2):int(nx/2 + d/2)+l] = -7.5
...
def representar_segments_llargs(equipotencials, salt=2, llargada=0.5):
    ...
    rectangle_1 = patches.Circle((4.1, 0), 0.4, facecolor='black', label='Fil a -7.5 V')
    plt.gca().add_patch(rectangle_1)

    rectangle_1 = patches.Circle((-4.1, 0), 0.4, facecolor='black', alpha=0.5, label='Fil a 7.5 V')
    plt.gca().add_patch(rectangle_1)
    ...
    # Crear la llegenda
    plt.legend(handles=[segment_experimental, fletxa_computacional]+plt.gca().get_legend_handles_labels()[0], fontsize=12, loc='upper left')
